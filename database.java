...
public class database {
  public static boolean DeBug = true;
    ...
    if (DeBug){//if debug is on, it prints the yes or no variable
      System.out.println("variable yesorno = " + yesorno);
    }
if(true==yesorno.contains("n")){//tests if you entered yes or no, and if no, it writes to UserData
    try {
      ...
      for(int i=1;i<=5;i++){
        ...
        try {
            ...
            if (DeBug){//if debug is on, it prints the a variable when there is an error with the name
    ...
    System.out.println("List of zip codes:");
      ...
      while (st.hasMoreTokens()) {//reads the tokens and prints them
            ...
            if (DeBug){
              System.out.println("variable tokn = " + tokn);//if debug is on, it prints the tokn variable before it gets processed
            }
          if(false==tokn.contains("\"")){//tests if the words have quotes
            ...
            if(true==tokn.contains("*")){//counts the number of yes's and no's
                if(tokn.contains("y")){
                  ...
                  if (DeBug){//if debug is on, it prints the yes or no variable as tokn
                    System.out.println("variable tokn as a yes or no = " + tokn);
                  }
                }
            }
